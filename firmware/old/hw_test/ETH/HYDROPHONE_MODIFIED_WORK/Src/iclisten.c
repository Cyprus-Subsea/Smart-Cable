...
struct time_message_type time_message;


uint32_t epoch_counter[]={0,0,0,0,0};
...
uint16_t crc_update( char data, uint16_t accum )
{
  ...
  comb_value = ( accum&0x0ff ) ^ (unsigned char)data;
...
void     prepare_timesync_message(struct time_message_type* message,uint32_t unixtime)
{
 ...
 message->sync=0x2a;
 message->type=0x41;
 message->payload_len=0x0004;
 message->ntime=unixtime;
 message->crc16=get_crc16((uint8_t*)message,8);
}

uint8_t check_epoch_message(uint8_t* buffer,uint8_t len)
{	 
	uint8_t detected_epoch_id=0;
	detected_epoch_id=parse_epoch(buffer, len);
	if(detected_epoch_id>0) 
	{
		epoch_counter[detected_epoch_id-1]++; 
		return 1;
	}
	return 0;
}
